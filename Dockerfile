...
# 1. Cài đặt ImageMagick + các thư viện ảnh cần thiết
...
# 2. Cấu hình Nginx với caching mạnh (giữ nguyên logic cũ của bạn)
RUN echo 'server { \
    listen 80; \
    root /var/www/html; \
...
# 3. Lưu trữ code gốc & Chuẩn bị môi trường (giữ nguyên logic cũ)
...
# 4. CMD: Kích hoạt hệ thống (đã tích hợp ImageMagick)
EXPOSE 80
CMD php-fpm -D && \
    # Thiết lập OpCache tối ưu
    ...
    # Cấu hình ImageMagick + các config quan trọng
